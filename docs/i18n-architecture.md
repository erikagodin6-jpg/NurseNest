...
- **Artifact:** `reports/i18n-status.json` (generated by `npm run i18n:status`, consumed by admin tooling when wired).
- **Server report builder:** `server/i18n-diagnostics-report.ts` powers `/api/admin/i18n-diagnostics` when enabled.
