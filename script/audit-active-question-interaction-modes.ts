...
function canonical(raw:string,mode:string):string{
  ...
  if(["mcq","multiple_choice","single_choice"].includes(t))return"MCQ";
  ...
  if(["image_based","image-based","hot_spot","hotspot","highlight"].includes(t))return"UNMAPPED_VISUAL";
  return"UNMAPPED";
}
...
console.log(JSON.stringify({sourceInteractionCounts:counts,canonicalMappings:mappings,unmappedSamples:samples},null,2));
