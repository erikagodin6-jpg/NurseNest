...
for(const file of ROOTS.flatMap(files)){
 ...
 const visit=(n:ts.Node)=>{if(ts.isObjectLiteralExpression(n)){
   ...
   if(found.length){const lc=sf.getLineAndCharacterOfPosition(n.getStart(sf));for(const[code,field,value]of found){issues.push({file,line:lc.line+1,id,field,text:value.slice(0,240),code});byCode[code]=(byCode[code]||0)+1;byFile[file]=(byFile[file]||0)+1;}}
...
console.log(JSON.stringify({blockingTemplateIssues:issues.length,byCode,largestFiles,sample:issues.slice(0,300)},null,2));
