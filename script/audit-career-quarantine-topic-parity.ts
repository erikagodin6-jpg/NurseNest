...
for(const entry of CAREER_QUESTION_QUARANTINE){if(!entry.requireTopicParity)continue;const legacy=topics(path.resolve(entry.legacyPath)),replacement=topics(path.resolve(entry.replacementPath));const missing=[...legacy].filter(t=>!covered(t,replacement)).sort();missingTotal+=missing.length;results.push({career:entry.career,legacyTopics:legacy.size,replacementTopics:replacement.size,coveredLegacyTopics:legacy.size-missing.length,coveragePercent:legacy.size?Math.round(((legacy.size-missing.length)/legacy.size)*10000)/100:100,missingTopics:missing});}
console.log(JSON.stringify({careers:results,missingTopics:missingTotal,complete:missingTotal===0},null,2));
if(missingTotal)process.exitCode=2;
