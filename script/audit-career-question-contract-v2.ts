...
for(const file of collect(ROOT)){
  ...
  const visit=(node:ts.Node)=>{if(ts.isObjectLiteralExpression(node)){
    ...
    if(initialStem&&initialOptions&&initialOptions.length>=2&&initialRationale){ordinal++;const career=careerFrom(file),id=String(parsed.id||`${career}-${ordinal}`),raw=applyCareerQuestionCorrection({...parsed,id});const stem=String(raw.stem||raw.question||"").trim(),options=Array.isArray(raw.options)?raw.options:[],rationale=String(raw.rationale||raw.rationaleCorrect||"").trim();const q=normalizeLegacyClientQuestion(raw,ordinal-1,jurisdiction(career));const issues:string[]=[];
      ...
      const combined=`${stem} ${rationale} ${options.join(" ")}`;for(const [code,re] of CLINICAL_REVIEW_PATTERNS)if(re.test(combined))issues.push(code);
