...
function collect(dir:string):string[]{if(!fs.existsSync(dir))return[];const out:string[]=[];for(const e of fs.readdirSync(dir,{withFileTypes:true})){const f=path.join(dir,e.name);if(e.isDirectory())out.push(...collect(f));else if(/-questions\.(ts|tsx)$/.test(e.name))out.push(f);}return out;}
...
function prop(n:ts.ObjectLiteralExpression,k:string):ts.Expression|null{for(const p of n.properties)if(propName(p)===k&&ts.isPropertyAssignment(p))return p.initializer;return null;}
...
function careerFrom(file:string):string{return path.basename(file).replace(/-questions\.(ts|tsx)$/,'');}
...
for(const file of collect(ROOT)){
  ...
  const visit=(node:ts.Node)=>{if(ts.isObjectLiteralExpression(node)){
    const raw=object(node);const stem=String(raw.stem||raw.question||"").trim();const options=Array.isArray(raw.options)?raw.options:null;const rationale=String(raw.rationale||raw.rationaleCorrect||"").trim();
    if(stem&&options&&options.length>=2&&rationale){ordinal++;const career=careerFrom(file);const id=String(raw.id||`${career}-${ordinal}`);const q=normalizeLegacyClientQuestion({...raw,id},ordinal-1,jurisdiction(career));const issues:string[]=[];
      ...
      const combined=`${stem} ${rationale} ${(options||[]).join(" ")}`;
      for(const [code,re] of CLINICAL_REVIEW_PATTERNS)if(re.test(combined))issues.push(code);
      rows.push({file,id,career,difficulty:Number(raw.difficulty),status:q.metadataOrigin,distractorOrigin:q.distractorMetadataOrigin||"unknown",issues:[...new Set(issues)],stem});
    }
  }ts.forEachChild(node,visit);};visit(sf);
}

const byCareer:Record<string,any>={};const issueCounts:Record<string,number>={};
...
const largest=Object.entries(byCareer).map(([career,v]:any)=>({career,...v})).sort((a,b)=>b.total-a.total);
const blockingRows=rows.filter(r=>r.issues.length);
console.log(JSON.stringify({totalQuestions:rows.length,authoredV2:rows.filter(r=>r.status==="authored-v2").length,legacyDerived:rows.filter(r=>r.status!=="authored-v2").length,issueCounts,byCareer:largest,sampleBlocking:blockingRows.slice(0,300)},null,2));
