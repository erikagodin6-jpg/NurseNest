...
- **Multi-Profession Framework**: Dynamic system for configuring new healthcare professions. Allied Health pages are unified into the main NurseNest shell (single-app architecture, no hostname-based branching). Allied pages use `AlliedNavigation` (teal-branded with career dropdown) and `AlliedFooter` instead of the nursing Navigation/Footer. The `AlliedLayout` wraps catch-all allied routes, while profession authority hub pages (`profession-authority-hub.tsx`) conditionally use allied vs nursing nav/footer based on the profession slug. `AlliedSubNav` provides career-specific sub-navigation. Empty content modules (flashcards, mock exams, tools) show polished "Coming Soon" fallback states via `ComingSoonFallback` component.
