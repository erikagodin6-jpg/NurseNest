...
- **New Grad Clinical References**: Seven clinical reference lessons at `/newgrad/clinical-references` covering sepsis, DKA, hemorrhage protocol, ICU monitoring, cardiac emergencies, medication calculations, and clinical handoff. Each lesson includes overview, key concepts, clinical pearls, red flags, exam tips, and 20-25 interactive flashcards. Data in `client/src/data/newgrad/clinical-reference-content.ts`. Hub page at `clinical-references-page.tsx`, detail page at `clinical-reference-detail.tsx`. Routes include `/new-grad/clinical-references/*` redirects to `/newgrad/clinical-references/*`.
