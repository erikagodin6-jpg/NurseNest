...
- **i18n System**: 15 languages supported (en, fr, tl, hi, es, zh, ar, ko, pt, pa, vi, ht, ur, ja, fa). RTL support for Arabic, Urdu, Farsi. All 15 languages have 264 keys each, fully translated in native scripts. Home page is fully internationalized with ~170 t() calls. Navigation (desktop + mobile) is fully internationalized including all menu items, dropdown labels, section headers, simulator names, and tool names. Footer is fully internationalized. Canadian content banner stays in English for SEO. The `learningItems` array uses a `key` field for routing logic so translated `label` values don't break navigation.
