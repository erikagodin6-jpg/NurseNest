...
- **Flashcard Admin Content Studio**: Comprehensive admin interface (`/admin/flashcard-studio`) for managing tier-specific flashcard bank content. Features: full CRUD with all fields (tier, blueprint category, question stem, options editor, correct answer selector, rationale sections, clinical pearl, image attachment, lesson link, high-yield/foundational flags), content validation (reject without tier/rationale, warn on missing lesson link or image-topic mismatch), question bank to flashcard conversion (bulk convert examQuestions to flashcardBank entries with preview), CSV/JSON bulk import with validation feedback, filter by tier/topic/status/source/difficulty, tier preview mode, publish/unpublish/archive bulk actions, and high-yield/foundational flag management. Schema additions: `high_yield`, `is_foundational`, `blueprint_category` columns on `flashcard_bank`.
