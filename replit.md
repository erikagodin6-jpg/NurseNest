...
- **Medical Imaging Education System**: Comprehensive exam prep for CAMRT and ARRT certifications, including lessons, positioning guides, physics reviews, flashcards, and practice exams. The Positioning Training Module features step-by-step learning mode (8-step wizard), interactive label overlays (CR, IR, landmarks, shielding, collimation, rotation), positioning error trainer with score tracking, quiz mode with multiple question types, image comparison (correct vs incorrect), and full admin CRUD. Routes: `/medical-imaging/:country/positioning` (listing) and `/medical-imaging/:country/positioning/:projectionSlug` (detail). Schema: `imaging_positioning_entries` with slug, country, bodyRegion, learningSteps/positioningErrors/quizQuestions/labelOverlays as JSONB.
