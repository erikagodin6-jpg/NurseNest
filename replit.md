...
- **Text Autofit**: `measureTextHeight()` uses offscreen canvas to compute true text height. `autofitText("expand"|"shrink")` fixes overflows by expanding box or shrinking font (min 8px). `findOverflows()` returns IDs; `highlightOverflows()` selects them. All in Brand Kit > Text Autofit section.
- **Store-Ready Pipeline**: `makeStoreReady()` runs in order: insert logo if missing → apply theme typography → normalize colors (if brandLock) → snap-to-grid + clamp margins → autofit text overflow → run audit + badge. Single undo snapshot. Primary CTA in Brand Kit > Quick Actions.
- **Multi-Page AI Pagination**: `insertAiBlocksMultiPage()` splits AI blocks across pages when content exceeds `CANVAS_HEIGHT - MARGIN`. Auto-creates new pages via API, adds "continued…" footer. `buildBlockObjects()` extracted helper for block→CanvasObject conversion. UI: "Multi-Page" button next to "Insert" in AI results.
- **Object Grouping**: `groupId?: string` on CanvasObject. `groupSelected()` assigns shared `gid()`. `ungroupSelected()` clears groupId from all grouped. Click selects entire group; shift-click toggles individual. Keyboard: Ctrl+G / Ctrl+Shift+G. Layers panel shows `[Group]` prefix. Group/Ungroup buttons in multi-select toolbar.
