...
### Overview
NurseNest is an adaptive learning platform designed for nursing and allied health students across 17 specializations. It provides extensive educational resources, advanced exam preparation (e.g., NCLEX, REX-PN), and performance analytics. The platform leverages AI for content generation to foster clinical reasoning, enhance nursing knowledge, and critical thinking, ultimately aiming to improve patient care outcomes and revolutionize nursing education through technology. It has a clear business vision to be a leader in health education tech, offering significant market potential.
...
- **Content & SEO Infrastructure**: An Allied Health Encyclopedia, SEO Lesson Engine, Programmatic SEO Engines, Multilingual SEO & Translation System, Database-Driven Multi-Domain Sitemap Architecture, Internal Linking Engine, and structured data generation (Article, Course, MedicalCondition, FAQ schemas) are implemented to enhance discoverability and content organization.
- **User Experience & Engagement**: Features like a Dashboard Lifecycle Command Center, Global Report a Problem System, IndexedDB-based Offline Study System, and LocalStorage-based Popup Suppression System improve user interaction and platform utility.
- **Multi-Profession Support**: A dynamic framework allows configuration for new healthcare professions with specialized navigation and content, including dedicated NP Exam Ecosystems.
- **Database Safety & Management**: PostgreSQL with Drizzle ORM is used, complemented by an EnvironmentAwareContentWriteService and a comprehensive Backup, Export & Restore Framework with real `pg_dump` backups, content exports, and integrity verification.
- **Content Integrity Engine**: This system ensures content quality through automated scanning, AI auto-repair, pre-publish validation, and a manual review queue.
- **Explanation Engine**: A unified structured explanation storage with AI-powered batch generation, quality scoring, and review workflow.
- **Exam Readiness Predictor Engine**: Provides readiness scores, pass probability, and personalized recommendations.
- **Unified Question Schema & Country Adaptation**: `exam_questions` table is extended to support international fields, filtering by country, language, and licensing body, with `country-adaptation.ts` for regional data mapping.
...
- **Platform Resilience System**: Enterprise-grade infrastructure providing circuit breakers, feature flags (with per-user/per-region/per-profession scoping), kill switches, health checks, rate limiting, load shedding, self-healing (cache corruption, missing backups, schema drift), emergency mode, provisional access, and entitlement caching. Includes client-side error boundaries and resilient fetch. Ops Dashboard at `/admin/ops` provides single-screen platform health overview with severity color-coding, auto-refresh, inline quick actions, and copy-to-clipboard status summaries. Mobile-optimized emergency panel at `/admin/ops/emergency` with large tap targets and confirmation dialogs. Key files: `server/platform-resilience.ts`.
