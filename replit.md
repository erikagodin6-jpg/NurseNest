...
- **Cross-Platform Analytics Tracking**: Tracks user flow between platform sections (exam_prep, new_grad, career_tools, allied_health) via `platform_section` dimension on page views, `cross_section_events` table for transitions, UTM parameter preservation across sections via sessionStorage, and an admin dashboard at `/admin/cross-platform` showing section traffic, transition flows, multi-section visitors, and education-to-career conversions. Route-to-section mapping defined in `shared/platform-sections.ts`.
...
- **SEO Implementation Progress Report**: Admin dashboard that aggregates content counts across all types (blog posts, SEO articles, practice pages, questions, flashcards, study guides, email subscribers) with per-profession coverage breakdowns.
- **Conversion Funnel System**: Reusable conversion funnel components (`ConversionFunnel`, `InlinePracticePreview`, `InlineFlashcardPreview`, `MockExamTeaser`, `PremiumFeatureSummary`, `SocialProofBar`) in `client/src/components/conversion-funnel/`. Integrated across lesson detail, question bank, flashcards, mock exams, new grad hub, and profession authority hub pages. Includes `/api/content-stats` endpoint for live content counts.
- **Career Journey Funnel Pages**: Guided pathway pages (`/career-journey` and `/career-journey/:slug`) that walk users through the student-to-professional pipeline: Exam Prep → Pass Exam → New Grad Transition → Career Launch. Generic and profession-specific variants (RN, RPN, Paramedic, RRT, MLT, Imaging, Pharmacy Tech, OT) with step-by-step resources, platform badges (NurseNest/NewGrad/ApplyNest), HowTo structured data, and CTAs on homepage, exam hubs, and new grad hub. Data config in `client/src/data/career-journey-data.ts`, page component in `client/src/pages/career-journey.tsx`.
...
- **Knowledge Graph Topic Pages**: Topic-level pages at `/topics` (index) and `/topics/:slug` (detail) that connect clinical concepts across all platform areas. Each topic page aggregates related lessons, practice questions, flashcards, new grad tips, career paths, and cross-profession perspectives. Topics are auto-derived from the internal links map (`client/src/data/internal-links.ts`). Organized by 15 body systems/specialties. Includes MedicalCondition or MedicalWebPage structured data (conditionally applied). Topic data model in `client/src/data/topics.ts`. All topic URLs included in sitemap.
