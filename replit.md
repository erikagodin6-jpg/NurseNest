...
## Paramedic Bulk Upload Manager
- Admin section at `/admin/paramedic-bulk-upload` for importing large amounts of paramedic content
- Supports JSON, CSV, and tabular text input via paste or file upload
- Content types: questions, flashcards, lessons, scenarios, glossary, SEO pages, study guides, practice exams
- Field mapping UI with auto-detection, manual mapping, and reusable saved templates
- Validation engine enforces `contentDomain = "paramedic"`, required fields, slug uniqueness, valid enum values, type-specific rules
- Normalization layer cleans smart quotes, spacing, line breaks without altering content
- Draft/preview/publish workflow with inline editing and revalidation
- Import history tracking with rollback capability for published batches
- Content Library view with type/status filters and bulk actions (publish/unpublish/archive/delete)
- All API endpoints enforce contentDomain filtering
- DB tables: `paramedic_bulk_imports`, `paramedic_bulk_import_items`, `paramedic_field_mapping_templates`
- Backend: `server/paramedic-bulk-upload.ts`
- Frontend: `client/src/allied/pages/paramedic-bulk-upload.tsx`
- Linked from allied admin page via "Paramedic Bulk Upload Manager" button
