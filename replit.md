...
- **Unified Certification Hub & Cluster Architecture**: Single authoritative hub at `/nursing-certifications` covering all emergency life-support certs (BLS, ACLS, PALS, NRP, TNCC, ENPC) and specialty certs (CCRN, CEN, CMSRN, etc.) with comparison table, study tools section, 8-question FAQ, and cross-links. 12 dedicated prep/renewal pages for 6 emergency certifications at `/certifications/{slug}-prep` and `/certifications/{slug}-renewal-prep`. Bidirectional linking: hub → prep → renewal → hub, with "Back to Certification Hub" links on all pages. New grad sub-hub at `/newgrad/certifications` links to the main hub. Newgrad cert detail pages (`/newgrad/certifications/{slug}`) include prep/renewal shortcuts and hub links. Content data in `client/src/data/certification-prep-content.ts`. 13 question banks in `client/src/data/exam-questions/certification-banks.ts`.
...
- **MLT (Medical Laboratory Technologist)**: 1,002 questions across 2 files. Base: `mlt-questions.ts` (500 template questions, mlt-001 to mlt-500). Batch 2: `mlt-questions-batch2.ts` (502 advanced clinical questions, mlt-501 to mlt-1002). Batch 2 covers: Hematology (102), Clinical Chemistry (109), Immunohematology/Blood Banking (71), Microbiology (68), Lab Operations & QC (51), Immunology/Serology (37), Urinalysis & Body Fluids (29), Molecular Diagnostics (20), Histotechnology (15). Questions include calculation-heavy content (MCV/MCH/MCHC, RPI, anion gap, osmolal gap, Friedewald LDL, creatinine clearance, dilutions, Westgard sigma metrics), case-based lab interpretation, and ASCP/CSMLS-relevant clinical scenarios. Aggregated in `career-question-pool.ts` under the `mlt` key.
