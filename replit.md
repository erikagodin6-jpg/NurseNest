...
- **Question Explanation Engine**: Unified structured explanation storage with AI-powered batch generation, quality scoring, review workflow, and migration utility.
- **Explanation Engine (Frontend)**: Structured post-answer explanations via `ExplanationPanel` component, including rationales, clinical context, key takeaways, and expandable sections.
- **Explanation Engine Admin Tools & Analytics**: Admin dashboard for explanation management, review queue, and content expansion roadmap.
- **Exam Readiness Predictor Engine**: Comprehensive readiness prediction and benchmarking backend, producing 0-100 readiness scores, pass probability estimation, and personalized recommendations.
