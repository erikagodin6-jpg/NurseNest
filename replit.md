...
- **Paramedic Lesson Content Library**: 52 AI-generated paramedic lessons across 10 EMS content domains (Airway Management, Trauma, Cardiology, Pharmacology, Respiratory, Medical Emergencies, Neurology, Pediatrics, Obstetrics, EMS Operations) stored as published `content_items`. Each lesson includes structured content blocks (Overview, Pathophysiology, Signs & Symptoms, Assessment, Prehospital Management, Complications, Clinical Pearls, Exam Pitfalls) with SEO metadata. 52 matching flashcard decks (416 total cards) linked via `flashcard_decks`/`deck_flashcards`. Cross-linked related lessons via `related_lessons` content blocks. Generation script at `server/seed-paramedic-lessons.ts`.
...
- **ICU/CCRN Critical Care Content**: 1,500+ ICU-level exam questions generated via V2 generator (gpt-4o-mini) covering 6 CCRN domains: Ventilator Management, Shock States, Hemodynamics, Sepsis, Trauma, ICU Pharmacology. Mix of MCQ (80%) and SATA (20%) at moderate-to-hard difficulty. ~70 static ICU flashcards across all 6 domains in `client/src/data/flashcards-icu-critical-care.ts`. Generation script: `server/scripts/generate-icu-questions.ts` (2-batch resume logic, generation IDs: `icu-ccrn-batch-1-*`, `icu-ccrn-batch-2-*`).
