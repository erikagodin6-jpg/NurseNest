...
- **i18n System**: 15 languages supported (en, fr, tl, hi, es, zh, ar, ko, pt, pa, vi, ht, ur, ja, fa). RTL support for Arabic, Urdu, Farsi. English has 714 keys; French has ~400 keys; other 13 languages have 266 keys with fallback to English for new keys. Pages with full i18n coverage: home (including Canadian banner), navigation, footer, FAQ, blog, mock exams, dashboard, lessons, flashcards, pricing (including tier features, trial passes, add-ons). The `learningItems` array uses a `key` field for routing logic so translated `label` values don't break navigation. The `t()` function falls back: `translations[language]?.[key] || translations.en[key] || key`.
