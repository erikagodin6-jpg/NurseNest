...
- **Memory Protection & Auto-Recovery System**: RSS-based memory monitoring with configurable thresholds (env vars: MEMORY_WARNING_MB, MEMORY_PROTECTION_MB, MEMORY_CRITICAL_MB). **Minimum production memory baseline: 2-4GB RAM.** Deployments with <512MB will experience chronic emergency mode cycling. All in-memory stores are hard-capped: metrics ring buffer (2000), alert/audit/event logs (100 each), rate limit store (1000), pageview tracking (1000), memory cache (200), entitlement cache (500), spike log (30), slow queries (50), abuse trackers (500), request trackers (500), i18n missing keys (500), VIP shed events (50). Health check uses RSS (not heapUsed/heapTotal ratio). Response body logging only retains 500-char snippets (not full objects). Load shedding middleware blocks bulk AI POSTs and caps list pagination under pressure. AI concurrency limited to 2 concurrent operations with 45s timeout and 512KB body limit. Response size limiter caps JSON responses. Cleanup sweeps prune stale sessions and caches. Emergency mode has a 10-min re-activation cooldown to prevent looping. Memory hysteresis requires 60s below 80% critical threshold before deactivation. Synthetic monitoring auto-pauses during emergency/protection mode with per-test exponential backoff (base 60s, max 30min). Non-essential heavy routes return 503 during emergency mode. Heavy workloads (blog batch generation, blog expand-all, bulk flashcard alignment, convert-to-flashcard, SM2 bulk generate) are deferred to the background job queue with registered handlers; the job queue worker pauses polling under memory pressure. Allied questions cache has TTL (30min) + LRU eviction + pressure pruning; paramedic cache has TTL (30min) + pressure pruning. Route instrumentation logs latency, payload size, and heap delta for critical routes. `/api/admin/memory-stats` exposes RSS, heap, route latency histogram, payload sizes, concurrent requests, job queue depth, and 30-point memory trend. `/api/process-health` provides lightweight health status. Frontend uses staleTime/gcTime for queryClient, auto-retries on 503, and an incident banner.
- **Alert Flood Prevention & Scaling Safety**: Global AlertCoordinator (`server/alert-coordinator.ts`) centralizes all alert email sending with per-incident deduplication, severity escalation tracking, and a global rate limit (~5 ops emails per 15-min window). Concurrency limiters (`server/concurrency-limiter.ts`) cap heavy routes: exam starts (10), exam loading (20), question bank (15), AI generation (3). Core user flows (login, dashboard, exam sessions, exams) are protected from load shedding during degradation. Request payload size limited to 2MB. Operator incident summary available at `GET /api/admin/incidents/summary`.
...
- **Worker Process Separation**: Background work (job queue polling, content pipeline, reporting scheduler, scheduled content publishing) isolated into `server/worker.ts` via `PROCESS_ROLE=worker` env var. Web process (`PROCESS_ROLE=web`, default) only serves HTTP requests — no background intervals. Phase 2/3 seeding also deferred to worker unless `RUN_SEEDING=true`. New file: `server/process-role.ts` for role detection helpers.
- **Paginated Exam Delivery**: `server/exam-delivery.ts` provides paginated exam API: `POST /api/exam-delivery/start` (returns shell + creates DB-persisted attempt), `GET /api/exam-delivery/:id/questions?page=N` (paginated questions), `GET /api/exam-delivery/:id/rationale/:qid` (on-demand rationale after answer), `POST /api/exam-delivery/:id/answer`, `POST /api/exam-delivery/:id/submit`. Concurrent assembly capped at 3. State stored in `exam_attempts` table.
- **Byte-Size Bounded Cache**: `server/performance.ts` memoryCache now has LRU eviction with both entry count (200) AND byte-size cap (50MB, configurable via `MEMORY_CACHE_MAX_BYTES`). Single entries >512KB are silently dropped. `clearCache()` export for emergency cache purge.
- **Memory Observability**: `server/memory-observability.ts` provides 30s ring buffer trend tracking, per-route heap/RSS deltas (for routes with >1MB delta or >100KB response), RSS baseline drift detection, top memory routes summary, and production sizing recommendations. Admin endpoint: `GET /api/admin/memory-observability`.
- **Enhanced Critical Mode**: Memory monitor now clears performance cache and stops synthetic monitoring when entering critical mode. Job queue skips poll cycles when memory pressure is active.
- **Lazy NurseNest Lite**: Prebuilt payloads in `server/nursenest-lite.ts` are lazily initialized on first access instead of eagerly at module load time.
