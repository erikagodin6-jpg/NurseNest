...
- **Platform Resilience System**: Enterprise-grade infrastructure providing circuit breakers, feature flags (with per-user/per-region/per-profession scoping), kill switches, health checks, rate limiting, load shedding, self-healing (cache corruption, missing backups, schema drift), emergency mode, provisional access, and entitlement caching. Includes client-side error boundaries and resilient fetch. Key files: `server/platform-resilience.ts`.
