...
- **Last-Known-Good Content Versioning**: Immutable versioning system for premium content. Every successful publish creates a version record with payload hash, version number, and validation status in the `content_versions` table. `getVerifiedContent()` returns the latest verified version with automatic failover to the previous verified version if runtime validation fails. Admin API endpoints for version history, inspection, and one-click restore. Integrated into all content publish flows: exam questions, flashcards, lessons, content items, and digital products. Key files: `server/content-version-service.ts`, `server/content-version-routes.ts`.
