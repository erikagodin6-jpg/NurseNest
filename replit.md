...
- **Subscription & Entitlement System**: Dedicated `user_subscriptions` table with planId, planName, billingInterval, status, activeFrom, expiresAt, renewsAt, canceledAt, trialStart, trialEnd, purchaseSource, lastVerifiedAt, stripeSubscriptionId, stripeCustomerId. Auth endpoints: `POST /api/auth/signup`, `POST /api/auth/logout`, `GET /api/auth/me` (returns `{user, subscription, entitlements}`). Standalone endpoints: `GET /api/me/subscription`, `GET /api/me/entitlements`. Admin: `POST /api/admin/grant-test-premium`. Entitlements return: isPremium, canAccessQuestions, canAccessFullBank, canAccessLessons, canAccessFlashcards, canAccessAnalytics, allowedRoles, allowedExamTypes. User model extended with firstName, role, country, exam, onboardingCompleted. Stripe webhooks (checkout.session.completed, invoice.paid, customer.subscription.updated, customer.subscription.deleted) update the user_subscriptions table. Key files: `server/subscription-routes.ts`, `shared/schema.ts` (userSubscriptions table).
