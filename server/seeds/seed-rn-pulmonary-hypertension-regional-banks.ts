...
import { rnCaPulmonaryHypertensionPublishedBank } from "../../client/src/data/exam-questions/rn-ca-pulmonary-hypertension-published";
import { rnUsPulmonaryHypertensionPublishedBank } from "../../client/src/data/exam-questions/rn-us-pulmonary-hypertension-published";
import "../../client/src/data/lessons/respiratory-rn-pulmonary-hypertension-coverage";
...
const cells: Array<{
  ...
  { label: "RN/CA/NCLEX-RN/Pulmonary Hypertension", countryCode: "CA", regionScope: "CAN", labUnitVariant: "SI", medicationNamingVariant: "Canadian_generic", questions: rnCaPulmonaryHypertensionPublishedBank },
  { label: "RN/US/NCLEX-RN/Pulmonary Hypertension", countryCode: "US", regionScope: "US", labUnitVariant: "conventional", medicationNamingVariant: "USAN", questions: rnUsPulmonaryHypertensionPublishedBank },
];
...
async function seed(cell: (typeof cells)[number]) {
  if (cell.questions.length !== 20) throw new Error(`RN_PH_SEED_COUNT_INVALID:${cell.label}`);
  const distribution = [0, 0, 0, 0];
  cell.questions.forEach((question) => distribution[question.correctAnswer] += 1);
  if (distribution.join(",") !== "5,5,5,5") throw new Error(`RN_PH_SEED_BALANCE_INVALID:${cell.label}/${distribution.join(",")}`);
  let inserted = 0;
  let skipped = 0;
  for (const question of cell.questions) {
    if (question.countryCode !== cell.countryCode || question.regionScope !== cell.regionScope || question.topic !== "Pulmonary Hypertension") {
      throw new Error(`RN_PH_SEED_SCOPE_INVALID:${question.id}`);
    }
    const stemHash = hash(question.stem);
    const existing = await pool.query(
      `SELECT id FROM exam_questions WHERE (stem_hash=$1 OR lower(trim(stem))=lower(trim($2))) AND tier='rn' AND exam='NCLEX-RN' AND region_scope=$3 AND topic='Pulmonary Hypertension' LIMIT 1`,
  ...
  if (row.total !== 20 || row.correct_rationales !== 20 || row.option_rationales !== 20 || row.difficulty_valid !== 20) {
    throw new Error(`RN_PH_POSTSEED_INVALID:${cell.label}/${JSON.stringify(row)}`);
  }
...
async function run() {
  ...
  if (verified !== 40) throw new Error(`RN_PH_TOTAL_INVALID:${verified}`);
