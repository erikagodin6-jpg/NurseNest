...
import { rpnCaAsthmaRexpnBankBatch1 } from "../../client/src/data/exam-questions/rpn-ca-asthma-rexpn-bank-batch1";
...
async function seedRpnCaAsthmaRexpnBankBatch1() {
  console.log("=== RPN Canada Asthma REx-PN Bank Batch 1 ===");
  await ensureRationaleColumns();

  if (rpnCaAsthmaRexpnBankBatch1.length !== 20) {
    throw new Error(`RPN_CA_ASTHMA_EXPECTED_20_QUESTIONS: ${rpnCaAsthmaRexpnBankBatch1.length}`);
  }

  let inserted = 0;
  let skipped = 0;

  for (const q of rpnCaAsthmaRexpnBankBatch1) {
    if (q.optionRationales.length !== q.options.length) {
      throw new Error(`RPN_CA_ASTHMA_OPTION_RATIONALE_MISMATCH: ${q.id}`);
    }
    if (!q.correctAnswerExplanation.trim()) {
      throw new Error(`RPN_CA_ASTHMA_CORRECT_RATIONALE_MISSING: ${q.id}`);
    }
    q.optionRationales.forEach((rationale, index) => {
      if (!rationale.trim()) {
        throw new Error(`RPN_CA_ASTHMA_OPTION_RATIONALE_MISSING: ${q.id}/${index}`);
      }
    });
    ...
    const incorrectRationales = optionRationaleRecord(q.options, q.optionRationales);
    ...
    await pool.query(
      `INSERT INTO exam_questions (
        ...
        $5, $6, $7,
        $8, $9, $10,
        $11, 'Respiratory', 'Asthma', 'CA', 'NCSBN',
        'en', $12, 'MCQ', 'SI',
        'Canadian_generic', 'CAN', true,
      ...
      [
        ...
        q.correctAnswerExplanation,
        JSON.stringify(incorrectRationales),
        q.clinicalReasoning,
        q.clinicalPearl,
        q.keyTakeaway,
        q.difficulty,
  ...
  `, [rpnCaAsthmaRexpnBankBatch1.map((q) => stemHash(q.stem))]);
  ...
  if (
    ...
    throw new Error(`RPN_CA_ASTHMA_POSTSEED_VERIFICATION_FAILED: ${JSON.stringify(row)}`);
  }

  console.log(`Inserted: ${inserted}`);
  console.log(`Skipped existing: ${skipped}`);
  console.log("Verified: 20 Canadian REx-PN asthma questions with complete rationale contracts");
  console.log("=== RPN Canada Asthma REx-PN Bank Batch 1 Complete ===");
}

seedRpnCaAsthmaRexpnBankBatch1()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error("RPN Canada asthma REx-PN seeding failed:", error);
    process.exit(1);
  });
