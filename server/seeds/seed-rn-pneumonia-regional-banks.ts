...
import "../../client/src/data/lessons/respiratory-rn-pneumonia-coverage";
...
const cells: Cell[] = [
  {
    label: "RN/CA/NCLEX-RN/Community-Acquired Pneumonia",
    countryCode: "CA",
    regionScope: "CAN",
    medicationNamingVariant: "Canadian_generic",
    labUnitVariant: "SI",
    questions: rnCaPneumoniaNclexBankBatch1
  },
  {
    label: "RN/US/NCLEX-RN/Community-Acquired Pneumonia",
    countryCode: "US",
    regionScope: "US",
    medicationNamingVariant: "USAN",
    labUnitVariant: "conventional",
    questions: rnUsPneumoniaNclexBankBatch1
  }
];
...
async function seedCell(cell: Cell) {
  if (cell.questions.length !== 20) {
    throw new Error(`RN_CAP_SEED_COUNT_INVALID: ${cell.label}/${cell.questions.length}`);
  }

  const distribution = [0, 0, 0, 0];
  cell.questions.forEach((question) => (distribution[question.correctAnswer] += 1));
  if (distribution.join(",") !== "5,5,5,5") {
    throw new Error(`RN_CAP_SEED_BALANCE_INVALID: ${cell.label}/${distribution.join(",")}`);
  }
  ...
  for (const question of cell.questions) {
    if (
      ...
      throw new Error(`RN_CAP_SEED_SCOPE_INVALID: ${cell.label}/${question.id}`);
    }
    ...
    const existing = await pool.query(
      `SELECT id
        ...
        WHERE (stem_hash = $1 OR lower(trim(stem)) = lower(trim($2)))
          ...
          AND exam = 'NCLEX-RN'
          AND region_scope = $3
          AND topic = 'Community-Acquired Pneumonia'
        LIMIT 1`,
    ...
    await pool.query(
      `INSERT INTO exam_questions (
        ...
        $5, $6, $7, $8, $9, $10,
        $11, 'Respiratory', 'Community-Acquired Pneumonia', $12, 'NCSBN',
  ...
  if (
    ...
    throw new Error(`RN_CAP_POSTSEED_INVALID: ${cell.label}/${JSON.stringify(row)}`);
  }
...
async function run() {
  ...
  if (verified !== 40) throw new Error(`RN_CAP_TOTAL_INVALID: ${verified}`);
