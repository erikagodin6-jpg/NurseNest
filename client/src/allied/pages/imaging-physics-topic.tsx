...
import {
  ChevronRight, ChevronDown, ChevronUp, Atom, BookOpen, AlertTriangle,
  Lightbulb, Stethoscope, CheckCircle2, XCircle, Eye, EyeOff, Zap, Brain
} from "lucide-react";
...
const SEED_TOPIC_DETAILS: Record<string, any> = {
  ...
  "inverse-square-law": {
    ...
    quizItems: [
      { question: "If distance is doubled, radiation intensity becomes:", options: ["1/2 of original", "1/4 of original", "1/8 of original", "2× original"], correctIndex: 1, rationale: "According to the inverse square law, I₁/I₂ = (D₂/D₁)². When distance doubles (D₂=2D₁), the ratio is 4, so new intensity is 1/4 of original." },
      { question: "A radiograph is taken at 40\" SID with 10 mAs. What mAs is needed at 72\" SID to maintain density?", options: ["18 mAs", "24 mAs", "32.4 mAs", "40 mAs"], correctIndex: 2, rationale: "Using the density maintenance formula: mAs₂ = mAs₁ × (D₂/D₁)² = 10 × (72/40)² = 10 × 3.24 = 32.4 mAs." },
      { question: "The inverse square law is based on which geometric principle?", options: ["Linear proportionality", "Area increases as the square of distance", "Volume increases as the cube", "Circumference increases linearly"], correctIndex: 1, rationale: "X-rays diverge from a point source. The area they cover increases as the square of the distance (area = 4πr²), so intensity per unit area decreases as 1/distance²." },
    ]
  }
};
  ...
  if (completed) {
    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-100 p-6 text-center space-y-4" data-testid="quiz-results">
        ...
        <button onClick={handleRestart} className="px-6 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700" data-testid="button-restart-quiz">
          Try Again
        </button>
      </div>
    );
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4" data-testid="micro-quiz">
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-gray-900 flex items-center gap-2">
          <Brain className="w-5 h-5 text-blue-500" /> Micro-Quiz
        </h3>
        <span className="text-sm text-gray-500" data-testid="text-quiz-progress">{currentQ + 1} / {quizItems.length}</span>
      </div>
      <div className="w-full bg-gray-100 rounded-full h-1.5">
        <div className="bg-blue-500 h-1.5 rounded-full transition-all" style={{ width: `${((currentQ + 1) / quizItems.length) * 100}%` }} />
      </div>
      ...
      {showRationale && (
        <button onClick={handleNext} className="px-5 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700" data-testid="button-quiz-next">
...
export default function ImagingPhysicsTopic() {
  const params = useParams<{ country: string; topicSlug: string }>();
  ...
  return (
    <>
      <AlliedSEO
        title={`${topic.title} - Radiation Physics | ${countryLabel} | NurseNest`}
        description={topic.seoDescription || topic.seo_description || `Learn ${topic.title} for the ${countryLabel} exam. Interactive visuals, micro-quiz, exam traps, and memory aids.`}
        keywords={`${topic.title}, radiation physics, ${countryLabel}, radiography, x-ray physics`}
        canonicalPath={`/medical-imaging/${country}/physics/${topicSlug}`}
        structuredData={{
          "@context": "https://schema.org",
          "@type": "EducationalCourse",
          name: topic.title,
          description: topic.seoDescription || `Interactive radiation physics lesson on ${topic.title}`,
      ...
      <div className="max-w-4xl mx-auto px-4 py-8" data-testid="physics-topic-page">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-xs">{countryLabel}</span>
          </div>
          ...
          {factorRelationships.length > 0 && (
            <section className="bg-white rounded-2xl border border-gray-100 p-6">
              <button
                ...
                className="w-full flex items-center justify-between"
                data-testid="button-toggle-relationships"
              >
                <h2 className="font-bold text-gray-900 text-lg flex items-center gap-2">
                ...
                <div className="mt-4 space-y-2">
                  {factorRelationships.map((r: any, i: number) => (
                    <div key={i} className="flex items-center gap-3 bg-gray-50 rounded-xl p-3 text-sm">
                      <span className="font-medium text-blue-700 min-w-[80px]">{r.factor1}</span>
