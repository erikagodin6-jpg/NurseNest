...
export default function ClinicalLabValuePage() {
  ...
  if (error || !page) return (<><Navigation /><div className="min-h-screen flex items-center justify-center" data-testid="lab-not-found"><div className="text-center"><h1 className="text-2xl font-bold text-gray-900 mb-2">Page Not Found</h1><p className="text-gray-600">The lab value page you are looking for does not exist.</p></div></div><Footer /></>);
      ...
      <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
        <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
          <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6" data-testid="breadcrumb-nav">
            ...
            <span>/</span>
            <span className="text-gray-500">Lab Values</span>
            <span>/</span>
          ...
          <header className="mb-8">
            <div className="flex items-center gap-2 mb-3">
              <span className="px-3 py-1 bg-indigo-50 text-indigo-700 text-xs font-semibold rounded-full">Lab Value Guide</span>
          ...
          {(data.normalRangeCA || data.normalRangeUS) && (
            <section className="mb-8" data-testid="section-normal-ranges">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-2xl font-bold text-gray-900">Normal Ranges</h2>
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                <div className="bg-white border border-gray-200 rounded-xl p-6 text-center">
                  <p className="text-xs text-gray-500 uppercase font-semibold mb-2">Canada (SI Units)</p>
                  <p className="text-2xl font-bold text-primary">{data.normalRangeCA}</p>
                </div>
                <div className="bg-white border border-gray-200 rounded-xl p-6 text-center">
                  <p className="text-xs text-gray-500 uppercase font-semibold mb-2">United States (Conventional)</p>
              ...
              {(data.criticalLow || data.criticalHigh) && (
                <div className="mt-3 bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-4 justify-center text-sm">
                  ...
                  {data.criticalLow && data.criticalLow !== "N/A" && <span className="text-red-800"><strong>Critical Low:</strong> {data.criticalLow}</span>}
                  {data.criticalHigh && data.criticalHigh !== "N/A" && <span className="text-red-800"><strong>Critical High:</strong> {data.criticalHigh}</span>}
                </div>
              )}
            </section>
          )}
          ...
          {data.clinicalSignificance && (
            <section className="mb-8" data-testid="section-clinical-significance">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-2xl font-bold text-gray-900">Clinical Significance</h2>
              </div>
          ...
          {data.interpretation && (
            <section className="mb-8" data-testid="section-interpretation">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-2xl font-bold text-gray-900">Clinical Interpretation</h2>
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                {data.interpretation.elevated && (
                  <div className="bg-white border border-red-200 rounded-xl p-6">
                    <h3 className="font-semibold text-red-800 mb-3 flex items-center gap-2"><TrendingUp className="w-5 h-5" /> Elevated (High)</h3>
                    <div className="space-y-4">
                      <div>
                        <p className="text-xs font-semibold text-gray-500 uppercase mb-1">Common Causes</p>
                      ...
                      {data.interpretation.elevated.ecgChanges?.length > 0 && (
                        <div>
                          <p className="text-xs font-semibold text-gray-500 uppercase mb-1">ECG Changes</p>
                      ...
                      <div>
                        <p className="text-xs font-semibold text-gray-500 uppercase mb-1">Nursing Actions</p>
                ...
                {data.interpretation.decreased && (
                  <div className="bg-white border border-blue-200 rounded-xl p-6">
                    <h3 className="font-semibold text-blue-800 mb-3 flex items-center gap-2"><TrendingDown className="w-5 h-5" /> Decreased (Low)</h3>
                    <div className="space-y-4">
                      <div>
                        <p className="text-xs font-semibold text-gray-500 uppercase mb-1">Common Causes</p>
          ...
          {data.relatedConditions && data.relatedConditions.length > 0 && (
            <section className="mb-8" data-testid="section-related-conditions">
              <h2 className="text-xl font-bold text-gray-900 mb-3">Related Conditions</h2>
