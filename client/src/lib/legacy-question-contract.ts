...
function hash(v:string):string {
  // FNV-1a 32-bit: browser-safe, deterministic, and sufficient for option-local identity when combined with question id/index.
  let h=0x811c9dc5;
...
function existingDistractors(raw:any, options:LegacyContractOption[], correct:Set<string>):Record<string,string>{
  const source=raw.distractorRationales||raw.distractor_rationales||raw.incorrectAnswerRationale||{};
...
export function normalizeLegacyClientQuestion(raw:any,index:number,jurisdiction:JurisdictionHint={}):LegacyContractQuestion {
  ...
  const authoredCorrect=text(raw.correctAnswerExplanation)||text(raw.correct_answer_explanation);
  const correctAnswerExplanation=authoredCorrect||rationale;
  const authoredHint=text(raw.hint)||text(raw.examStrategy)||text(raw.exam_strategy);
  ...
  const authoredWhy=text(raw.whyThisMatters)||text(raw.why_this_matters)||text(raw.keyTakeaway)||text(raw.key_takeaway);
  ...
  const authoredPearl=text(raw.clinicalPearl)||text(raw.clinical_pearl)||text(raw.examPearl);
  ...
  const distractorRationales=existingDistractors(raw,options,correctSet);
  ...
  const authoredV2=options.every(o=>!!o.id)&&answers.length>0&&!!authoredCorrect&&!!authoredHint&&!!authoredWhy&&!!authoredPearl&&Object.keys(existingDistractors(raw,options,correctSet)).length===Math.max(0,options.length-correctSet.size);
  ...
  return {
    ...
    whyThisMatters,
    clinicalPearl,
    mnemonic:text(raw.mnemonic)||text(raw.memoryHook)||undefined,
    countryCode:text(raw.countryCode)||text(raw.country_code)||jurisdiction.countryCode,
    countryLabels:Array.isArray(raw.countryLabels)?raw.countryLabels:jurisdiction.countryLabels,
    regionScope:text(raw.regionScope)||text(raw.region_scope)||jurisdiction.regionScope||"GLOBAL",
    languageCode:text(raw.languageCode)||text(raw.language_code)||jurisdiction.languageCode||"en",
    exam:text(raw.exam)||jurisdiction.exam||raw.exam,
    licensingBody:text(raw.licensingBody)||text(raw.licensing_body)||jurisdiction.licensingBody,
    optionContractVersion:2,
    publicationContractVersion:2,
